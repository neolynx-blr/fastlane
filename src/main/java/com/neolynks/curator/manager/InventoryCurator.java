...
public class InventoryCurator {
	...
	public void processNewInventory() {
		...
		LOGGER.info(
			...
			LOGGER.warn(
					"Exception [{}] with message [{}] occurred while looking for any new data arriving in the vendor-item-master table. Eating the exception for it will be retried soon.",
						...
						for (VendorItemMaster latestItemData : latestItemDataList) {
							...
							if (previousDifferentialInventoryData.getAddedItems().keySet().contains(itemBarcode)) {
								...
								if (CollectionUtils.isNotEmpty(oldItemDataList)) {
									VendorItemMasterWrapper vimWrapper = new VendorItemMasterWrapper(oldItemDataList.get(0));
									oldItemInfo = vimWrapper.generateVIHItemInfo();
								}
