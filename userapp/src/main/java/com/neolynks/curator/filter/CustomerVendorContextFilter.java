...
import com.neolynks.curator.util.CustomerAPIRequestResponseUtil;
...
public class CustomerVendorContextFilter implements ContainerRequestFilter, ContainerResponseFilter {
    ...
    public void filter(ContainerRequestContext requestContext) throws IOException {
        ...
        if (!StringUtils.isBlank(requestToken)) {
            UserVendorContext userVendorContext=  CustomerAPIRequestResponseUtil.getUserVendorContext(requestToken);
            UserContextThreadLocal.getUserVendorContextLocale().set(userVendorContext);
            MDC.put(USER_ID_KEY, requestToken);
        }
    }
